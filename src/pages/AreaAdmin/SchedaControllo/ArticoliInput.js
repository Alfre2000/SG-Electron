...
function ArticoliInput() {
  const articoliQuery = useCustomQuery({ queryKey: URLS.ARTICOLI_NESTED_PAGINATED });
  const results = articoliQuery.data?.results;

  const { initialData } = useFormContext();

  const articoliWithSchedaQuery = useCustomQuery(
    { queryKey: URLS.ARTICOLI + `?scheda_controllo=${initialData?.id}` },
    { enabled: !!initialData?.id, onSuccess: (data) => setArticoli(data)}
  );

  const [modalArticolo, setModalArticolo] = useState(false);
  const [ricerca, setRicerca] = useState("");
  const [cliente, setCliente] = useState(null);
  const [articoli, setArticoli] = useState(!!initialData ? articoliWithSchedaQuery.data || [] : []);
  ...
  const articoliPossibili = results
    ? results.filter((articolo) => {
        ...
        const articoloNomeFilter =
          articolo?.nome?.toLowerCase()?.includes(ricerca) || articolo?.codice?.toLowerCase()?.includes(ricerca);
  ...
  return (
    <>
      {modalArticolo && (
        <ModifyModal show={modalArticolo} handleClose={() => setModalArticolo(false)}>
          <Form
      ...
      <Row className="mb-4 text-center">
        <Col xs={5}>
          ...
          <ListGroup className="text-sm max-h-[250px] h-[250px] overflow-scroll rounded-t-none border-t-0 border border-slate-300">
            {articoliQuery.isLoading && <Loading className="relative top-20" />}
        ...
        <select type="hidden" name="lista_articoli" multiple className="hidden">
          {articoli &&
            articoli.map((articolo) => <option key={articolo.id} value={articolo.id} selected={true}></option>)}
          <option></option>
        </select>
      </Row>
    </>
